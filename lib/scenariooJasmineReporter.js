...
ScenariooJasmineReporter.prototype.reportSpecResults = function (spec) {
    ...
    console.log("scenario :: " + spec.description.name + " :: " + (result.passed() ? "passed" : "failed"));
    if (!result.passed()) {
        ScenarioDocuWriter.saveStep("Failure");
    }
    var currentScenario = {
        useCaseName: spec.suite.description.name,
        scenarioName: spec.description.name,
        scenarioDescription: spec.description.description,
        stepCounter: 0,
...
/**
 * is invoked at the end of a useCase
 */
ScenariooJasmineReporter.prototype.reportSuiteResults = function (suite) {
    var result = suite.results();
    console.log("useCase :: " + suite.description.name + " :: " + (result.passed() ? "passed" : "failed"));
    var useCase = {
        name: suite.description.name,
